...
const TimeUnit: React.FC<{ value: number; label: string; rotate?: string }> = ({ value, label, rotate = 'rotate-0' }) => (
  <div className={`flex flex-col items-center mx-2 sm:mx-4 transform ${rotate}`}>
    <div className="relative">
        <div className="text-5xl sm:text-7xl font-marker text-slate-800 tabular-nums tracking-wide drop-shadow-sm">
    ...
    <span className="text-lg sm:text-xl font-hand text-slate-600 font-bold mt-2">
      {label}
    </span>
  </div>
);
...
const CountdownTimer: React.FC<CountdownTimerProps> = ({ timeLeft }) => {
  return (
    <div className="flex flex-wrap justify-center items-center py-6 px-4 bg-white/40 backdrop-blur-sm rounded-xl border-2 border-slate-800/5 shadow-sm transform -rotate-1">
      <TimeUnit value={timeLeft.days} label="Days" rotate="-rotate-2" />
      <span className="text-4xl sm:text-6xl font-marker text-slate-400 -mt-8 px-1">:</span>
