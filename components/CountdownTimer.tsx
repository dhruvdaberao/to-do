...
const TimeUnit: React.FC<{ value: number; label: string; rotate?: string }> = ({ value, label, rotate = 'rotate-0' }) => (
  <div className={`flex flex-col items-center mx-1 md:mx-4 transform ${rotate} shrink-0`}>
    <div className="relative">
        ...
        <div className="text-[10vw] sm:text-7xl md:text-8xl font-marker text-slate-800 tabular-nums tracking-wide drop-shadow-sm leading-none">
...
const CountdownTimer: React.FC<CountdownTimerProps> = ({ timeLeft }) => {
  return (
    <div className="flex flex-nowrap justify-center items-center py-4 sm:py-8 px-2 sm:px-8 bg-white/40 backdrop-blur-sm rounded-xl border-2 border-slate-800/5 shadow-sm transform -rotate-1 w-full overflow-hidden mx-auto">
